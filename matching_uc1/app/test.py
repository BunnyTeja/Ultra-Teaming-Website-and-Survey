...
def getValue():
    ...
    if input_method == 'M0: Random Matching':
      return render_template("index.html", method=input_method, use_case="uc_1")
    ...
    #     return(render_template("matching.html",name = it,length = len(proposal_ids_for_it),my_range = range(len(proposal_ids_for_it)),names = names, nsfids = proposal_ids_for_it, nsftitles = proposal_titles_for_it, nsflinks = proposal_links_for_it, years = proposal_years_for_it, itext = it, teams = new_team_list, method = input_method, gscore = new_gscore_list, no_of_teams = int(input_teams), no_of_results = int(input_results)))
